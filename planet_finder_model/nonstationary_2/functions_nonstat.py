...
def shared_core_pos(x, b, P, phi, c, T):
    # offset c keeps the core positive, max(0, .) is a safeguard, and
    # N = 1 + c + |b|*T bounds it to [0, 1] over a baseline t in [0, T]
    N = 1 + c + np.abs(b) * T
    return np.maximum(0.0, b * x + np.sin(2 * np.pi * x / P + phi) + c) / N

def shared_core_pos_and_grads(x, b, P, phi, c, T):
    arg = 2 * np.pi * x / P + phi
    N = 1 + c + np.abs(b) * T
    raw = b * x + np.sin(arg) + c
    active = (raw > 0).astype(float)
    core = np.maximum(0.0, raw) / N

    dcore_db = active * (x / N - core * np.sign(b) * T / N)
    dcore_dP = active * (-(2 * np.pi * x / (P**2 + 1e-12)) * np.cos(arg)) / N
    dcore_dphi = active * np.cos(arg) / N
    dcore_dc = active * (1 - core) / N

    return core, dcore_db, dcore_dP, dcore_dphi, dcore_dc

def alpha_cyc(t, b, P, phi, c, T):
    return shared_core_pos(t, b, P, phi, c, T)

def alpha_cyc_grad(t, b, P, phi, c, T):
    _, dcore_db, dcore_dP, dcore_dphi, dcore_dc = shared_core_pos_and_grads(t, b, P, phi, c, T)
...
def make_nonstat_kernel(T, b, P, phi, c):
    # T is bound via partial so it is a fixed constant, not a hyper-parameter
    return NonStationaryKernel(partial(alpha_cyc, T=T), partial(alpha_cyc_grad, T=T), b=b, P=P, phi=phi, c=c)
...
def negloglike_nonstat(theta, t_full, y_full, series_index,C, rv_std = 1.0, inject_planet=False):
  ...
  core0, dcore0_db, dcore0_dP, dcore0_dphi, dcore0_dc = shared_core_pos_and_grads(t_full[series_index[0]], b, P, phi, c, T)
  core1, dcore1_db, dcore1_dP, dcore1_dphi, dcore1_dc = shared_core_pos_and_grads(t_full[series_index[1]], b, P, phi, c, T)
  ...
  idx_c = params.index('rot.nonstat_c')

  # b, P, phi, c also enter the mean term directly, on top of the covariance
  ...
  base_grad[idx_c] += (
        np.sum(dL_dy[series_index[0]] * a0 * dcore0_dc) +
        np.sum(dL_dy[series_index[1]] * a1 * dcore1_dc))
...
def log_prior_nonstat(theta, bounds_list, planet = True):
    if planet == True:
        ...
        rv_jit, rhk_jit, b, P, phi, c, rot_P0, rot_rho, rot_eta, rot_alpha_0, rot_alpha_1, rot_beta_0, a0, a1, d0, d1 = theta
...
def plot_fit_nonstat(t_full, y_full, yerr_full, series_index, C, xbest, rv_std = 1.0, output_name = 'fit_plot.png', return_residuals=True, inject_planet=True):
    ...
    for k in range(2):
        ...
        b, P, phi, c = xbest[idx_b], xbest[idx_P], xbest[idx_phi], xbest[idx_c]
        core0 = shared_core_pos(t_full[series_index[0]], b, P, phi, c, T)
        core1 = shared_core_pos(t_full[series_index[1]], b, P, phi, c, T)
...
def plot_kernel_draw(t_full, C, xbest, sig=1.0, n_points=2000, output_name='kernel_draw.png'):
    ...
    C_draw = cov.Cov(
        ...
        rot=term.SimpleProductKernel(
            nonstat=make_nonstat_kernel(np.max(t_full), b, P, phi, c),
