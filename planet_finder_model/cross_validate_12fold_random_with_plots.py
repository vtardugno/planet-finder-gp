...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--plot-dir", default="results/cv_optim_plots", help="Directory where optimisation plots are saved")
    return parser
    ...
    for cycle_fit in (False, True):
        ...
        for fold_id, test_idx in enumerate(folds, start=1):
            ...
            try:
                ...
                plot_name = Path(args.plot_dir) / f"{mode_name}_fold{fold_id:02d}.png"
                mf.plot_fit(
                    train_fold.t_full,
                    train_fold.y_full,
                    train_fold.yerr_full,
                    ...
                    best_xbest,
                    rv_std=train_fold.rv_std_raw,
                    output_name=str(plot_name),
                    return_residuals=False,
                    inject_planet=args.fit_planet,
                )
            finally:
