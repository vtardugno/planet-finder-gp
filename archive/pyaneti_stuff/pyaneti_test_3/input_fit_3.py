...
telescopes_labels = ['nadege RV','nadege log $R_{HK}$']
...
#kernel_rv = 'None'
fit_krv = ['f']*7 #Define the list with 9 elements
fit_krv[0] = 'u' #A_0
fit_krv[1] = 'u' #A_1
fit_krv[2] = 'u' #A_2
...
#if 'g' is selected for the parameter A, sets gaussian priors with mean min_A and standard deviation max_A
krv_priors = [     
        0.0,5.0,   #ranges for A_0
        -5.0,5.0,   #ranges for A_1
        -5.0,5.0,   #ranges for A_2
        0.0,0.0,   #ranges for A_3, note that this one is fixed to zero
        
        1,100,      #ranges for lambda_e
        0.1,3.0,   #ranges for lambda_p
        17.,26.0     #ranges for P_GP
        ]
